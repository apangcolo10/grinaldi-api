...
$id_guru = intval($_GET['id_guru'] ?? 0);
if ($id_guru <= 0) { echo json_encode(['status'=>'error','message'=>'id_guru tidak valid']); exit; }

$hari = isset($_GET['hari']) ? $conn->real_escape_string(trim($_GET['hari'])) : '';

// Coba tabel "jadwal" dulu, kalau tidak ada pakai "data_jadwal"
$tabel = 'jadwal';
...
$sql = "SELECT j.id_jadwal, j.hari, j.jam_ke, j.waktu_mulai, j.waktu_selesai,
               k.nama_kelas, m.kode_mapel, m.nama_mapel, g.nama_guru
        FROM $tabel j
        JOIN kelas k ON j.id_kelas = k.id_kelas
        JOIN mapel m ON j.kode_mapel = m.kode_mapel
        JOIN guru g ON j.id_guru = g.id_guru
        WHERE j.id_guru = $id_guru";
if ($hari !== '') $sql .= " AND j.hari = '$hari'";
